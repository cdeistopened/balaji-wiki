#!/usr/bin/env python3
"""
Interlink Balaji Wiki — add [[wiki-links]] for first occurrence of known entities.
...
- Strips frameworks/ prefix from wiki links
"""
...
def extract_titles(content_dir: Path) -> dict[str, tuple[str, str]]:
    ...
    For frameworks: uses title field from .md file
    For skills: uses name field from SKILL.md in subdirectory, slug is directory name
    """
    titles = {}

    # Process frameworks (*.md files directly in frameworks/)
    ...
    if frameworks_dir.exists():
        for md_file in frameworks_dir.glob("*.md"):
            ...
            # For titles with colons, also register the short version
            if ":" in title:
    ...
    # Process skills (subdirectories with SKILL.md files)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Interlink Balaji Wiki pages")
        ...
        for filepath in files:
            ...
            if count > 0:
                ...
                if args.verbose:
                    ...
                    print(f"  {parent_dir}/SKILL.md: +{count} links")

        total_links += dir_links
        print(f"\nskills/: {dir_links} links added across {len(files)} files")
